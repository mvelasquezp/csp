...
<html>
	<head>
		<title>Subida de artículos - Radio</title>
		<meta charset="utf-8" />
		...
		<style type="text/css">
			* { box-sizing: border-box; }
			...
			#audio-container{background:#404040;display:table;height:100%;padding:20px;width:100%;}
			#audio-container>div{display:table-cell;margin:0 auto;text-align:center;vertical-align:middle;}
			#audio-container>div>audio{display:inline-block;}
		</style>
		...
		<script type="text/javascript">
			...
			function load_audio() {
				var a = $(this);
				myWindow = dhxWins.createWindow("win-audio", 0, 0, 640, 240);
				myWindow.center();
				...
				$.post("{{ url('ajax/data/if_audio') }}", p, function(response) {
					myWindow.progressOff();
					if(response.success) {
						var audio_prop = response.prop;
						myWindow.setText("Revisando audio | " + audio_prop.articulo + " [De: " + audio_prop.medio + "]");
						...
						myWindow.attachHTMLString('<div id="audio-container"><div></div></div>');
						$("#audio-container").children("div").eq(0).append(
							$("<audio/>").attr({id:"audio-file", controls: true, autoplay: true}).append(
								$("<source/>").attr({src:response.source, type:"audio/mpeg"})
			...
			function populate_audios(audios) {
				...
				for(var i in audios) {
					...
					$("#articulos-container").append(
						$("<a/>").attr({
							...
							$("<div/>").append(
								$("<p/>").append($("<b/>").html(audio.nom))
							).append(
								$("<p/>").html(audio.nmd)
							).append(
								$("<p/>").html("Publicado: " + audio.fup)
							)
						).on("click", load_audio)
					);
				}
			...
			function initComponents(data) {
				...
				myLayout.cells("a").setText("Registrar archivo de sonido");
				myLayout.cells("b").setText("Archivos de audio del día");
				myLayout.cells("b").attachHTMLString('<div id="articulos-container"></div>');
				//cargar audios
				var audios = data.files;
				populate_audios(audios);
							...
							$.ajax({
								...
								success: function(response) {
									if(response.success) {
										...
										//recargar audios
										$.post("{{ url('ajax/components/genera-articulos-radio') }}", { _token: "{{ csrf_token() }}" }, function(rsp) {
											if(rsp.success) {
												populate_audios(rsp.data.files);
											}
											else dhtmlx.alert({
												title: "Error",
												type: "alert-error",
			...
			function init() {
				$.post("{{ url('ajax/components/genera-articulos-radio') }}", { _token: "{{ csrf_token() }}" }, function(response) {
					if(response.success) {
