...
<html>
	<head>
		<title>Subida de artículos - Redes Sociales</title>
		<meta charset="utf-8" />
		...
		<script type="text/javascript">
			...
			var config = {
				formData: [
					...
					{type: "input", label: "Autor", labelWidth: 65, name: "tbAutor", validate: "NotEmpty"},
						...
						case "btGuarda":
							...
							if(imagen.length > 0) {
								...
								var p = {
									...
									aut: myForm.getItemValue("tbAutor"),
									ttl: myForm.getItemValue("tbTitulo"),
									txt: myForm.getInput("tbTexto").value,
									kws: myForm.getItemValue("tbKeywords")
								};
								$.post("{{ url('ajax/data/sv_screenshot_rs') }}", p, function(response) {
			...
			function init() {
				$.post("{{ url('ajax/components/genera-articulos-redes') }}", { _token: "{{ csrf_token() }}" }, function(response) {
					if(response.success) {
